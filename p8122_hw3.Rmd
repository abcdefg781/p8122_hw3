...
The average causal effect of participating in a job training on wages is `r ace`, which is positive, so we can say that job training has a positive effect on wage. The confidence interval for the average causal effect is (`r CIL`, `r CIU`), which is a wide range, so we cannot be completely confident that the average causal effect is positive. The p-value of the average causal effect is 
